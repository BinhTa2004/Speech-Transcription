...
def preprocess_audio(filepath, sr=16000, n_mels=128, max_duration=10):
    """
    ...
    log_mel = log_mel.repeat(3, 1, 1)  # [3, 128, 313]
    #log_mel = log_mel.unsqueeze(0)  # shape: [3, n_mels, time_steps]
...
def generate_caption_optimized(model, mel, tokenizer, max_len=30, device='cpu', beam_size=3):
    ...
    with torch.no_grad():
        encoder_out = model.encoder(mel)  # (1, time_steps, encoder_dim)
        ...
        if beam_size == 1:
            ...
            for _ in range(max_len):
                emb = model.decoder.embed(input_word).squeeze(1)  # (1, embed_dim)

                # Apply attention here
                context, alpha = model.decoder.attention(encoder_out, h.squeeze(0))  # (1, encoder_dim)
                lstm_input = torch.cat((emb, context), dim=1).unsqueeze(1)  # (1, 1, embed + encoder_dim)
            ...
            for _ in range(max_len):
                ...
                for seq, score, (h, c) in sequences:
                    ...
                    emb = model.decoder.embed(input_word).squeeze(1)

                    context, alpha = model.decoder.attention(encoder_out, h.squeeze(0))
                    lstm_input = torch.cat((emb, context), dim=1).unsqueeze(1)

                    output, (new_h, new_c) = model.decoder.lstm(lstm_input, (h, c))
                    logits = model.decoder.fc(output.squeeze(1))
